...
void write(robotInit* data, short number, byteArray* byte_ptr){
    char* array;
    ...
    for(int i = 0; i < number; ++i){
        ...
        char* url = data[i].VideoURL->c_str;
        ...
        //push the string
        for(j = 16; j < 16+str_len; ++j){
            structs[i][j] = ref[j - 16];
        }
...
void write(robotUpdate* data, short number, byteArray* byte_ptr){
    char* array;
    unsigned short* sizes = new unsigned short[number]; 
    int overall_size = 0;
    char** structs = new char*[number];
